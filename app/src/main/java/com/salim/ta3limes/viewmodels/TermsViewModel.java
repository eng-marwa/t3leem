...
public class TermsViewModel extends ViewModel {
    ...
    public void GetTermsAndConditions(){
        ...
        ServiceInterfaces.GetTerms getTerms = builder.getTerms();
        Call<TermsModelResponse> call = getTerms.getTerms("Bearer " + token);
        call.enqueue(new Callback<TermsModelResponse>() {
            @Override
            public void onResponse(Call<TermsModelResponse> call, Response<TermsModelResponse> response) {
                Log.e(TAG, "onResponse: Response >> " + response);
                if (response.code() == 200){
                    progress.setValue(false);
                    String content = response.body().getData().getTerms().getContent();
            ...
            @Override
            public void onFailure(Call<TermsModelResponse> call, Throwable t) {
